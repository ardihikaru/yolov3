...
class Plot:
    ...
    def run(self):
        ...
        # self.__comparison_graph_v1()
        self.__comparison_graph_v2()

    def __overall_graph(self):
        time_inference_latency = read_data(self.latency_output, self.csv_inference)
        time_nms_latency = read_data(self.latency_output, self.csv_nms)
    ...
    def __comparison_graph_v1(self):
        comp_path = self.latency_output + "/comparison/"
        c_time_inference_latency = read_data(self.latency_output, self.csv_inference)
        c_time_nms_latency = read_data(self.latency_output, self.csv_nms)
        c_time_mbbox_latency = read_data(comp_path + "custom", self.csv_mbbox)

        # get total latency
        custom_latency = []
        default_latency = []
        for i in range(len(c_time_inference_latency)):
            custom_latency.append( c_time_inference_latency[i] + c_time_nms_latency[i] + c_time_mbbox_latency[i] )
            default_latency.append( c_time_inference_latency[i] + c_time_nms_latency[i] )
    ...
    def __comparison_graph_v2(self):
        comp_path = self.latency_output + "/comparison/"
